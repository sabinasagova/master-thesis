...
def plot_ressource_view(
    ...
    for i in range(len(list_resource)):
        patches = []
        for j, polygon in enumerate(polygons_ax[i]):
            x, y = polygon.exterior.xy
            ax[i].plot(x, y, zorder=-1, color="b")
            patches.append(pp(xy=polygon.exterior.coords))
            mid_x = (x[0] + x[2]) / 2
            mid_y = (y[0] + y[2]) / 2
            ax[i].text(mid_x, mid_y, labels_ax[i][j], ha="center", va="center")
...
def plot_precedence_relations(rcpsp_model: RCPSPModel, title_figure: str = "") -> plt.Figure:
    import networkx as nx
    nx.drawing.nx_pylab.draw_networkx(
        rcpsp_model.graph.to_networkx(),
