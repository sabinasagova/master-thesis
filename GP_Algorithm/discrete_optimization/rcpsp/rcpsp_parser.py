...
def parse_psplib(input_data: str) -> RCPSPModel:
    ...
    except ValueError: # Compatibility code for MMLIB
        duration_ref_line_index = lines.index("REQUESTS/DURATIONS")
    ...
    renewable_resources = [
        name for name in list(resources.keys()) if name.startswith("R")
    ]
    ...
    for i_line in range(duration_start_line_index, duration_end_line_index + 1):
        ...
        if tmp[0].startswith("***"): # Compatibility code for MMLIB
            break
    ...
    return RCPSPModel(
        resources=resources,
        renewable_resources_list=renewable_resources,
        non_renewable_resources_list=non_renewable_resources,
...
def parse_file(file_path: str) -> RCPSPModel:
    with open(file_path, "r", encoding="utf-8") as input_data_file:
        ...
        rcpsp_model.file_path = file_path #TODO: Fix later
        return rcpsp_model
