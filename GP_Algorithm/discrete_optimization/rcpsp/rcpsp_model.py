...
class RCPSPModel(Problem):
    ...
    def __init__(
        ...
        renewable_resources_list: List[str],
        non_renewable_resources_list: List[str],
        ...
        **kwargs: Any,
        ...
        self.resources_list: list[str] = list(self.resources.keys())
        if renewable_resources_list is None:
            ...
            self.renewable_resources_list = renewable_resources_list
        if non_renewable_resources_list is None:
            non_renewable_resources_list = [
                res
                for res in self.resources_list
                if res.startswith("N")
            ]
        else:
            self.non_renewable_resources_list = non_renewable_resources_list
        self.non_renewable_resources_list = non_renewable_resources_list
        ...
        if horizon is None:
            self.horizon = sum(
                [
                    max([
                        self.mode_details[act][m]["duration"]
                        for m in self.mode_details[act]
                    ]
                    )
                    for act in self.mode_details
                ]
            )
        else:
    ...
    def evaluate_from_encoding(
        ...
        if encoding_name == "rcpsp_permutation":
            ...
            raise NotImplementedError(
                f"Encoding {encoding_name} not implemented")
    ...
    def get_attribute_register(self) -> EncodingRegister:
        ...
        max_number_modes = max([len(self.mode_details[x])
                               for x in self.mode_details])
    ...
    def compute_resource_consumption(
        ...
        makespan = rcpsp_sol.rcpsp_schedule[self.sink_task]["end_time"]
        consumptions = np.zeros(
            (len(self.resources), makespan + 1), dtype=np.int_)
        for act_id in rcpsp_sol.rcpsp_schedule:
    ...
    def plot_ressource_view(self, rcpsp_sol: RCPSPSolution) -> None:
        ...
        for i in range(len(self.resources_list)):
            ax[i].axhline(
                y=self.resources[self.resources_list[i]
    ...
    def copy(self) -> "RCPSPModel":
        model = RCPSPModel(
            ...
            non_renewable_resources_list=self.non_renewable_resources_list,
            renewable_resources_list=self.renewable_resources_list,
            ...
            mean_resource_reserve=self.costs.get(
                "mean_resource_reserve", False),
    ...
    def set_fixed_attributes(self, encoding_str: str, sol: RCPSPSolution) -> None:
        att = self.get_attribute_register(
        ).dict_attribute_to_type[encoding_str]["name"]
        if att == "rcpsp_modes":
...
def create_np_data_and_jit_functions(
    ...
    predecessors = np.zeros(
        (rcpsp_problem.n_jobs, rcpsp_problem.n_jobs), dtype=np.int_)
    successors = np.zeros(
        (rcpsp_problem.n_jobs, rcpsp_problem.n_jobs), dtype=np.int_)
    ...
    ressource_renewable = np.ones(
        (len(rcpsp_problem.resources_list)), dtype=bool)
    ...
    for k in range(len(rcpsp_problem.resources_list)):
        ...
        if rcpsp_problem.resources_list[k] in rcpsp_problem.non_renewable_resources_list:
...
def evaluate_constraints(
    ...
    list_constraints_not_respected = compute_constraints_details(
...
def compute_constraints_details(
    ...
    for (t1, t2, off) in start_at_end_plus_offset:
        ...
        if not b:
            list_constraints_not_respected += [
                ("start_at_end_plus_offset", t1, t2,
            ...
            if solution.get_end_time(t) < constraints.end_times_window[t][0]:
                list_constraints_not_respected += [
                    (
                        ...
                        None,
                        # type: ignore
                        constraints.end_times_window[t][0] - \
...
def check_solution_with_special_constraints(
    ...
    for (t1, t2, off) in start_at_end_plus_offset:
        ...
        if not b:
            logger.debug(
                ("start_at_end_plus_offset NOT respected: ", t1, t2, off))
            logger.debug(
                (
