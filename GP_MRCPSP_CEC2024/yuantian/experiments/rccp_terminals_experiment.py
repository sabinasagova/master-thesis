"""
RCCP terminals vs baseline, with cp_propagation as the comparison that
actually matters here.

cp_propagation.py came back as basically a null result (faster
convergence, no quality gain, see readme.md extension #1) because those
terminals turned out to just be linear combos of ES/EF/LS/LF -- nothing
new for the GP to use. RCCP terminals are resource-state-dependent
instead, which ES/EF/LS/LF genuinely can't express. This experiment is the
direct test: does resource-driven criticality info actually help where
precedence-only CP info didn't?

Four conditions:
  - baseline: normal terminal set
  - cp_propagation: + CP_FORWARD, CP_BACKWARD, CP_SLACK_SCORE, CP_PROB
  - rccp: + RCCP_BOTTLENECK_UTIL, RCCP_CANDIDATE_CONTENTION, RCCP_SLACK,
    RCCP_PRESSURE_TREND
  - both: both terminal sets together

Scaled down from the paper defaults so this fits on a laptop, same scale
as cp_propagation_experiment.py so the tables line up.

Run with (from the repo root):
    PYTHONPATH=$(pwd):$(pwd)/yuantian python3 yuantian/experiments/rccp_terminals_experiment.py
"""
import json
import random
import sys
...
# as cp_propagation_experiment.py)
# ---------------------------------------------------------------------------
N_SEEDS = 10
SEED_BASE = 8000
POP_SIZE = 60
...
CONDITIONS = ("baseline", "cp_propagation", "rccp", "both")

# Same train/test instances as cp_propagation_experiment.py, so this table
# lines up with that extension's existing reported numbers.
...
OUTPUT_DIR = Path(__file__).parent / "results" / "rccp_terminals_experiment"
...
def build_params(condition: str) -> ParametersGPHH:
    params = ParametersGPHH.fast(
        ...
        cp_propagation_feature=condition in ("cp_propagation", "both"),
...
def analyze(all_results: list):
    ...
    for metric, metric_label in [("best_fitness", "training fitness"), ("test_fitness", "test fitness")]:
        print(f"\n-- {metric_label} --")
        for condition in ("cp_propagation", "rccp", "both"):
            report_pair("baseline", condition, metric)
        print("  -- direct comparison: precedence-only CP vs resource-driven RCCP --")
        report_pair("cp_propagation", "rccp", metric)
