"""
Reference implementation of Modifications 1-7 of the exploratory phase
(thesis Section on DMGE screening): the lazy if_then_else operator, the
first NR terminals, the schedule-state and mode-flexibility terminals, the
critical-path-preserving mutation registry, the backward serial SGS hook,
CP_EXTENSION_IF_SCHEDULED, and the urgency/regret terminals.

NOTE: this module is written against the legacy simulator interface of the
superseded GP_Algorithm codebase (its FeatureEnum members and
mutCriticalPathPreserving do not exist in the current package), so it is
kept for documentation and is not imported by the active pipeline. The
graft vocabulary actually used by the active screening lives in
exploratory/diagnostic_graft.py.
"""
